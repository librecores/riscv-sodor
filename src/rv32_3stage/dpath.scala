...
class DatPath(implicit val p: Parameters) extends Module 
{
   ...
   val rf_rs1_data = Mux((exe_rs1_addr =/= 0.U) , regfile(exe_rs1_addr), 0.U(xlen.W))
   val rf_rs2_data = Mux((exe_rs2_addr =/= 0.U) , regfile(exe_rs2_addr), 0.U(xlen.W))
   ...
   io.dmem.resp.ready := once | RegNext(io.ctl.dmem_fcn)
   if(p(NUM_MEMORY_PORTS) == 1) 
   ...
   val irt_reg = RegInit(0.U(xlen.W))
