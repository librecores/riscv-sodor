...
class DatPath(implicit p: Parameters) extends Module
{
   ...
   //Initialize IO
   io.dmem.req.bits := 0.U.asTypeOf(new MemReq(xlen))
   io.imem.req.bits := 0.U.asTypeOf(new MemReq(xlen))
   ...
   val exe_reg_pc_plus4 = RegInit(0.U(xlen.W))
   ...
   val if_pc_plus4 = (if_reg_pc + 4.U(xlen.W))               
   ...
   exe_alu_out := MuxCase(0.U, Array(
                  ...
                  (io.ctl.alu_fun === ALU_SLL)  -> ((exe_alu_op1 << alu_shamt)(xlen-1, 0)).asUInt,
   ...
   val tsc_reg = RegInit(0.U(xlen.W))
   tsc_reg := tsc_reg + 1.U
